...
describe("Quiz routes", () => {
  ...
  test("Show all quizes by user id", async() => {
    let user = await User.getByUsename('test_user1')
    const res = await request(app)
      .get(`/quizes/user/${user.id}`)
    expect(res.body).toEqual([
        {"category": "Nature", "id": expect.any(Number), "score": 5, "user_id": expect.any(Number)}, 
        {"category": "Books", "id": expect.any(Number), "score": 8, "user_id": expect.any(Number)},
  ...
  test("User did not take any quizes", async() => {
  ...
  test("Update score for the quiz", async() => {
    ...
    expect(quizes.body[0]).toEqual({"category": "Nature", "id": expect.any(Number), "score": 5, "user_id": expect.any(Number)})
    let quizId = quizes.body[0].id
