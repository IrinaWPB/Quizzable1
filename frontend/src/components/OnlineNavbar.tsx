...
const OnlineNavbar: React.FunctionComponent<IOnlineNavbarProps> = ({
  ...
  const [newOutgoingMessage, setNewOutgoingMessage] =
  ...
  const handleOnlineButtonClick = (): void => {
    if (online) {
      ...
      clientSocketInstance.emit("go_online", {
  ...
  const handleInvitationClick = (id: string): void => {
    clientSocketInstance.emit("send_invitation", {
      toId: id,
      fromId: playerId,
    });
    navigate("/multi-game");
  ...
  const handleSendMessage = (e: React.FormEvent<HTMLButtonElement>): void => {
    ...
    if (newOutgoingMessage.length > 0) {
      clientSocketInstance.emit("new_chat_message", {
        newOutgoingMessage,
        playerId,
        opponentId,
      });
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      clientSocketInstance.off(
        "updated_online_players",
        handleOnlinePlayerChange
      );
      ...
      clientSocketInstance.off("messages_not_read", () =>
        handleReadStatus(false)
      );
    };
  }, []);
  ...
  return (
    <>
        ...
        <div>
          <Menu.Item style={{ margin: "10px" }}>
            ...
            <Button
              ...
              size="large"
              content={online ? "Go Offline" : "Go Online"}
            />
          </Menu.Item>
        </div>
      </OnlineContainer>
      {activeItem === MenuState.friends &&
        Object.keys(onlinePlayers).length > 0 && (
          <OnlineContainer>
            <h1>Online Players</h1>
            <div className="onlinePlayers">
              {Object.keys(onlinePlayers).length > 0 ? (
                Object.keys(onlinePlayers).map(
                  (key) =>
                    onlinePlayers[key] !== currentUser.username && (
                      <div key={key}>
                        <Header>{onlinePlayers[key]}</Header>
                        <ButtonElement
                          actions={() => handleInvitationClick(key)}
                          text="Invite to play"
                        />
                        <ButtonElement
                          actions={() => handleStartChatClick(key)}
                          text="Start Chat"
                        />
                      </div>
                    )
      ...
      {activeItem === MenuState.messages && invitationFrom && (
        <OnlineContainer style={{ flexDirection: "column" }}>
          ...
          <div>
            <ButtonElement
              text="Join Game"
              actions={() => handleJoinGameClick(invitationFrom)}
            />
          </div>
          <div>
            <ButtonElement
              text="Cancel"
              actions={() => setInvitationFrom(null)}
            />
          </div>
        </OnlineContainer>
      )}
      ...
      {chatOpen && (
        <ChatContainer>
          <CloseButton onClick={() => closeChat(opponentId)}>
            <span>{currentUser.username}</span> X
          </CloseButton>
          <ScrollToBottom className="chat_body">
            {chatMessages &&
              chatMessages.map((m: Message) =>
                m.outgoing === false ? (
                  <div key={uuid()} className="opponents_message_wrapper">
                  ...
                  <div key={m.messageBody} className="my_message_wrapper">
                    <div className="my_message">
                      ...
                      <div className="timestamp">
                        <p>
                          {m.timestamp.getHours()}:{m.timestamp.getMinutes()}
                          {isRead && <span className="read">✔</span>}
                        </p>
                      </div>
                    </div>
                  </div>
